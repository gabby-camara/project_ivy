...
multiplot(cols = 3, plotlist = list(
...
plotlist = list(
fviz_nbclust( x=df_area, diss = d_prsn,
              pam,
              method = "wss") +
...
pam2 = pam(x = d_prsn, diss = TRUE, k=2)
pam3 = pam(x = d_prsn, diss = TRUE, k=3)
pam4 = pam(x = d_prsn, diss = TRUE, k=4)
...
```{r,fig.width=10, fig.asp=.25, echo=FALSE, warning=FALSE, message=FALSE}
  multiplot(cols = 3, plotlist = plotlist[[2]])
           
```

```{r,fig.width=10, fig.asp=.25, echo=FALSE, warning=FALSE, message=FALSE}
   multiplot(cols = 3, plotlist = plotlist[[3]])
...
```{r, fig.width=14, fig.asp=.25, echo=FALSE, warning=FALSE, message=FALSE}
...
Although there seems to be 2 clear clusters from a spatial point of view it doesn't seem to have a specific impact on the **Sale
...
```{r, fig.width=10, fig.asp=.25, echo=FALSE, warning=FALSE, message=FALSE}
hc_avg <- hclust( d = d_prsn, method = "average")
hc_complete <- hclust( d = d_prsn, method = "complete")
hc_single <- hclust( d = d_prsn, method = "single")
hc_wd <- hclust( d = d_prsn, method = "ward.D2")
# validation
# =================
...
coph_wd <- cophenetic(hc_wd) 
# Correlation between cophenetic distance and 
# the original distance 
cor( d_prsn, coph_avg) %>%
  print(sprintf('Correlation: Average Method = %d', .))
cor( d_prsn, coph_complete) %>%
  print(sprintf('Correlation: Complete Method = %d', .))
cor( d_prsn, coph_single) %>%
  print(sprintf('Correlation: Single Method = %d', .))
cor( d_prsn, coph_wd) %>%
  print(sprintf('Correlation: Ward Method = %d', .))
# Agerage seems to be tge better option
...
rm(hc_complete, hc_single, hc_wd, coph_complete, coph_single, coph_wd)
```
